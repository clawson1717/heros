...
class TestCriticIntegration:
    def test_planner_critic_integration(self):
        ...
        from heros.planner import Milestone, SubgoalPlan

        # Create a plan manually (LLM not needed for integration test)
        plan = SubgoalPlan(
            task="Write a quicksort function in Python",
            milestones=[
                Milestone(
                    id="m1",
                    description="Write the function signature and docstring",
                    rubric="Function has a signature with correct name and a docstring",
                    expected_output="def quicksort(arr): ...",
                ),
                Milestone(
                    id="m2",
                    description="Implement the quicksort algorithm",
                    rubric="Code contains partition logic and recursive calls",
                    expected_output="Partition and recursion implemented",
                ),
            ],
        )
